...
public class ApiAccessController {
    ...
    /**
     * 1. 本地访问；http://localhost:8080/authorize?username=rainyak&password=123
     * 2. 云服务访问；http://api.xfg.im:8080/authorize?username=rainyak&password=123
     * 3. 内网穿透；在docs/natapp/natapp执行；通过获得的地址访问服务  http://xfg.nat300.top/authorize?username=rainyak&password=123
     */
    @RequestMapping("/authorize")
    public ResponseEntity<Map<String, String>> authorize(String username, String password) {
        Map<String, String> map = new HashMap<>();
        // 模拟账号和密码校验
        if (!"rainyak".equals(username) || !"123".equals(password)) {
            map.put("msg", "用户名密码错误");
    ...
    public String success(){
        return "test success by rainyak";
    }
